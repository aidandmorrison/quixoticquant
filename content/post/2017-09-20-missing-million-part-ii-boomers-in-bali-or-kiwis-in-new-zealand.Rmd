...
thumbnail: "images/IndoRegr2.png"
categories:
  - Demographics
  - Economics
  - Migration
...
But more interestingly, we can see that the short-term departures of all the trips under two months clearly can't add up to anything like a million people.  I did some calculations and found that in fact at the end of the data *all* the short-term departures probably only cumulatively add up to under 800,000.  I suspect the assumption of a flat distribution across all the categories is likely to overstate if anything.  Furthermore, at the end of 2001, before the Missing Million had left, those departures accounted for about 400,000 people absent.  So it really isn't plausible that all these departures listed actually account for anything like the full Missing Million, or pobably even half of that.  The sub-set of Boomers that Cameron Murray describes is doomed to be a trival minority in any case.  

There are a couple of reasons that this could be the case.  Probably most importantly is that the data presented here is based on people's indicated intent, as per their passenger departure cards.  There's absolutely no obligation on the traveller to honour that committment.  Often they may not have actually booked their onward or return flight.  If I had  to speculate a little as to the micro-economic dynamics that might be at work, I would think that plenty of travellers (particularly those on temporary visas in Australia) would simply indicate what they felt was most appropriate to their visa, regardless of true intentions, in order to raise the least possible questions.

But perhaps more importantly, given the way that migration is defined under the 12/16 rule, there's absolutely nothing preventing these 'short term' travels actually contributing to migration.  Two six-month trips a couple of months apart will constitute migration.  As will a two-week trip, if some was travelling a lot in the previous year. 

There's also the possibility of significant a shift internally within categories.  It could be the case that my estimation of a flat distribution across each time interval is false, and some change has led quite systematically to some of the categories to become increasingly  left-skewed.  The switching over of the 6-12 month and 3-6 month lines seems to show some evidence of that occurring, however if a shift in that direction also occurs internally within the categories, we woud likely have even less of the Missing Million accounted for. 

But perhaps the most important explanation is that this only looks at half of the story.  We have short-term arrivals aswell:
...
Here a couple of things are striking.  In particular, the under-1 week movement trend is actually the mostly the highest, ofllowed by 1-2 weeks.  People arriving in Australia seem to report a far shorter intended stay.  And, the the total number of arrivals is far far lower than the departures.  This is also consistent with the hypothesis I've outlined, earlier, that we're a hard pace to get to for a short trip, but a good place to leave from for one. 
...
1. The Missing Million grew to half it's height while travel to Indonesia was low and flat, and travel to New Zealand was increasing.  
...
(Note, one can click the countries in the legent to turn off a series, or double-click a country to show a single series.  This helps to isolate or inspect individual trends.)
...
From the above two charts it can be quite easily seen that the slump in departures growth that occurred in the mid-2000s, and most dramatic elevation in after about 2008, is in South East Asia and to some extent the USA. This is again consistent with Australia now competing with other attractive pacific desinations as holiday spots, but probably acting as something of a home-base for people wanting to explore South East Asia.  Arrive long-term to Australia, but leave short-term, frequently to explore Asia.
...
All this strongly points towards some significant overlap between the largest contributors to 'persons absent' due to departues being New Zealanders visiting friends or family back home, rather than boomers in Bali.

We could go further to try to substantiate that claim useing some more advanced statistical techniques.  For example, we could use a decomposing function to isolate the seasonal and trend components in a given time series, such as this version which does this using the loess smoother:
...
We could then compare the seasonal component (in the second pane) with other seasonal structures in different category types.  However, as can be seen from the assymetric errors in the bottom pain, the growth in the seasonality of the series isn't perfectly accounted for.  This highlights that the structure of the seasonality is subject to change aswell within the time-series.  Again, my preferred method to get a sense of how this might be the case is to use the Mk 1 eyeball again for a quick check.  Another visual represenation of the raw data is to overlay the different years on top of one another.
...
Here we can see why the regression came up with this result.  Departures to New Zealand slump in May each year, and 3-6 month departures have a peak in May.  Subracting that component consequently improves the fit substantially, though it's not a physically possible result.  Overall, it appears that my attempt to get these statistical methods to substantiate a proposed split between the differet visit duration seems to have failed.  It's probable that in New Zealand's case the implicit assumption (that the distrubtion of trip-lengths to new zealand is relatively stable over time) doesn't hold well. We'll relax this assumption a little later to see if it improves things, but first let's just check to see how close a more-realistic model can come.

To get a more realistic model, we can re-run the regression with a constraint that requies all the co-efficients to be non-negative.  Happily there's an R package called nnls by Katharine Mullen which can quickly create such a model.  Te result is this:
...
The graph also shows that we have a weighting of 6-12 months that is over 1, which is also unrealistic, since we can't have 120% of 6-12 month departures all going to New Zealand.  With more time and resources there would certainly be means of further constraining the models, including with the outputs of other regressions on other variables, to make a fuller and more consistent model. But first let's just multiply these 'weightings' by the mean actual number of travel undertaken under each departure lenth, and then by the average length of stay, to convert the coefficients back into numbers which are easier to make some sense of: numbers of departures monthly, and number of 'person-years' absent that could be attributable to that flow.
...
This graph shows the results of running three different multi-variate regressions on the three time-intervals indicated.  As you can see, the fits now look quite impressive.  However, the unconstrained model relied upon some unrealistic methods to get such good results:
...
To make the most of an uncertain insight, I'll again revert to my favourite technique, compare it to something else, or take a derivative, so we can at least see how different it is to something else.  Let's have a look at a similar regression model for departures to Indonesia.
...
This looks like a far better result.  We can see here that the component with the greatest statistical confidence (dark colour) is a positive component, and less than 1, which means that it's a realistic number.  The largest negative also have the least significance statistically (lighter colour).  Overall this looks far better.  Let's see how whether it translates to a decent model with only-positive coefficients.
...
Finally a result which we can put some more faith in.  Travel to Indonesia was dominated by quick trips when it was low in the 90s and 2000s.  Since 2010 the largest increase has also been in short trips, probably of 1-2 weeks duration.  However, it's likely that there's been a noticeable, and new component of longer trips.  Whilst substantially smaller in number, it's plausible that this shift towards longer-term visits accounts for even more absentees from Australia overall.  However, even combined, the growth in short term departures between the decades of the naughties and the teens could only account for about 50,000 people, around 5% of the Missing Million, and a neglible portion of the significant share that had grown in the years prior to 2010. 
...
```{r, fig.align="center", out.width = 600, fig.cap="The abandoned departure card assumed that people know whether they are 'resident' or 'termporary entrant'."}
...
Apparently the ABS is realising that the data collected on them wasn't adding up and isn't helping.  In particular, asking people to self-select what type of traveller they are, in particular whether they are a 'visitor or temporary entrant' or 'Australian resident' makes no sense for our increasinly part-time population, many of whom are foreign citizens, here on a temporary visa, but staying (as a Student, worker, or backpacker) for long enough to officially qualify to be 'resident' under a statistical definition they've never heard of.

Not only are people unlikely to accurately identify their category, but they have absolutely no obligation to honour their stated intention of travel plans. With more and more people making multiple stops on their travels, with few of them planned in advance, they're likely to make some educated guess about what the scary people at the customs gate are going to be most happy to hear. 

Furthermore, not all the data captured in departures cards is comprehensively enumerated.  According to the ABS, on average, [only about 5%](http://www.abs.gov.au/AUSSTATS/abs@.nsf/Lookup/3401.0Explanatory%20Notes1Jul%202017?OpenDocument) of the cards are selected for a sample, and most of the rest imputed.  Many of the methods I've used above could well struggle if there's substantiall errors or other poor assumptions made in this sampling or imputation process. The strange results produced by our New Zealand regression could be an indication of uncertainty or inconsistency in this process as much as anything else.

Overall I remain adamant, as I've [previously claimed](https://www.quixoticquant.com/post/the-state-of-debate-a-bird-s-eye-on-migration/), that immigration data is one of the most falsely-revered 'clean' statistics in existence.  The hardest numbers we have still show [evidence of substantial](https://www.quixoticquant.com/post/the-missing-million/) discrepancies, enough to radically alter the policy conclusions we have from them.  The more detailed numbers we tend to reach to construct narratives to defend our use of them generally don't add up, or aren't consistently or comprehensively recorded enough to indicate anything at all with any confidence.  I still maintain that the best narrative to explain the Missing Million is that the enormous increase in international mobility has, coupled with our peculaur economic, polical, and overwhelmingly geographical circumstance has left us with a 'part-time population', and a frothy figure of 'residents' that is consistently and substantially higher than the number of people we physically have here at any given time.  The nature of this dynamic needs a lot more work, and I welcome any further contributions.
