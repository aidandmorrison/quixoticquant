...
```{r NOM_12_16, warning=FALSE, message= FALSE}
...
```
This presentation gets good marks for integrity in your quantitative methods course.  It poses the proper question: Why does the ABS not have a longer data series on immigration? (Spoiler alert: it does, it's just not giving it to us.)
...
```{r, fig.align="center", out.width = 400, fig.cap="Prod Commission..."}
...
plot <- ggplot(joint_sheet_310101, aes(x = Date, text = as_date(Date)))+
  ...
  geom_text(x = as.numeric(sheet_340101$Date[date_change - 10]), y = 5000000, label =  "AUD below 
            0.85 USD", size =3)+
...
plot <- ggplot(joint_sheet_310101, aes(x = Date, text = as_date(Date)))+
  ...
  scale_colour_manual("", 
                      breaks = c("Natural Increase Prev 12 Months", "Net Overseas Migration Prev 12 Months", "Increase in ERP Prev 12 Months"),
...
Here the alternative to Australia's runaway migration program can be seen.  

It is possible that for some factors the NOM might be a relevant predictory, for example in property. Aussies are likely to hold onto their homes even while the spend more time overseas, and an investor who spends little time here can push up property prices. But the moment someone starts claiming that immigrants are congesting our roads and there's no way our infrastructure can keep up with their rate of arrival, (see Cory Bernardi and Mark Latham [here](https://www.youtube.com/watch?v=28ZO4BXRq-Q)), or read Macrobusiness any day of the week, and it's time to grip your sides and start rolling around in the aisles in laughter. _That_ particular rate of growth has unequivocally slowed.  People who are overseas don't drive their cars on the M4 in Sydney, or cram up the train stations in Melbourne.  

It's also ridiculous that the million officially resident - but presently absent - people constitute a ["growing reserve arbmy of labour"](http://www.abc.net.au/news/2017-07-12/population-growth-masking-australian-economic-weakness/8700288) that's bidding down the prices of local labour.  The reality is that at any given moment there's actually a million-strong army of non-present residents, many of whom are probably chasing higher incomes - but overseas. 

The mind boggles at the slew of other statistics that this could seriously affect if the ERP was replaced with PPP.  It might explain the consistently disappointing nominal GDP growth that Australia has been experiencing for some time, which assumes that a comfy 1.5% growth can come from population increases, before any productivity is added. And plenty of labour statistics are bound to be out aswell, since surveys of local addresses wouldn't tell us much about how hard the army of non-present residents actually work (or what they earn) while they're not here.  But ABS scales up the results, [using ERP](http://www.abs.gov.au/AUSSTATS/abs@.nsf/Lookup/6202.0Explanatory%20Notes1Jul%202017?OpenDocument), to include them in the labour force. Lacklustre wage growth, poor tax revenue, almost everything that the government relies on to make proper policy decisions could be woefully distorted, but exploring that mess will also have to wait until a later post.

It's also perfectly possible that plenty of other developed countries have also been effectively double-counting an increasing part of their populations.  In my searching of ABS definitions, I found no mention of needing to adhere to any international rules or standards to ensure that the world's population added up.  Perhaps the miscounting of populations could be added to the [list](https://www.vox.com/2016/8/1/12131216/theories-gdp-growth-slow) of possible explanations of low-growth and productivity decline in the Western World.  Depending on time, and reader interest, that may be one I'll explore further. 

But before signing off for now, I'll give the ABS one final lashing, in response to a nasty twist that emerged late in my research for this post, when trying to get confirmation of my hypothesis from the recent Census.  Yes, it turned out I was naive again.  The quick and obvious numbers from [ABS Quickstat](http://www.censusdata.abs.gov.au/census_services/getproduct/census/2016/quickstat/036) for 2016 and 2011 ("Persons count based on place of usual residence on Census night" which I've plotted below) actually aren't directly comparable to either the ERP or PPP.  But finding the ones that are turned out to be, well, not that easy.  Let's just say that ABS knows what it's like to realise they've been over-estimating population by a few hundred thousand people. [Last time that happened](http://www.abs.gov.au/AUSSTATS/abs@.nsf/Previousproducts/3101.0Feature%20Article3Jun%202012?opendocument&tabname=Summary&prodno=3101.0&issue=Jun%202012&num=&view=) (2011) they smeared the error out over the previous two decades so that no-one would notice. Unpicking that mess, and relating it to the observations above will be the subject of my next post.
